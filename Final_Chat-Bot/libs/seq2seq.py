...
class seq2seq_chatbot(object):
    def __init__(self, oneHot_size, l_r, vec_list, vec_size, enc_len, dec_len, n_layer1, n_hiddens, model_name):
        self.oneHot_size = oneHot_size
        self.l_r = l_r
        # word vectors constant
        self.vecs = tf.constant(vec_list)
        ...
        tf.Variable(True, dtype=tf.bool)
    ...
    def encoder(self, rnn_cell):
        ...
        enc_inp = [tf.nn.dropout(tf.sigmoid(tf.matmul(word_vec[i], w_enc)+b_enc), keep_prob=.6, name='enc_inp') \
        ...
        with tf.variable_scope('enc'):
            ...
            go = tf.add(tf.matmul(encoder_output[-1], w_enc_out), b_enc_out, name='go')
    ...
    def decoder_attention(self, rnn_cell, encoder_output, encoder_state, go):
        ...
        dec_inp = [tf.nn.dropout(tf.nn.relu(tf.matmul(target[i], w_dec)+b_dec), keep_prob=.6, name='dec_inp') \
        ...
        with tf.variable_scope('dec'):
            ...
            for i in range(self.dec_len):
                ...
                scores = [tf.nn.relu(tf.add(tf.matmul(tf.concat([encoder_output[j], dec_o], axis=1), w_scores), b_scores),
                                     name='score')
                ...
                weights = [tf.divide(scores[j]+(0.1/self.enc_len), sum_scores, name='atten_weight') for j in range(self.enc_len)]
                # context vector
                context_vec = tf.add_n([tf.multiply(weights[j], encoder_output[j]) for j in range(self.enc_len)], name='context_vec')
                # attention vector
                atten_vec = tf.add(tf.matmul(tf.concat([context_vec, dec_o], axis=1), w_atten_vecs), b_atten_vecs,
                                   name='word_output')
    ...
    def save_model(self, sess, name=time.strftime('%Y.%m.%d_%H%M%S')):
        return self.saver.save(sess, './model/'+self.model_name+'/'+name+'_chatbot')
    ...
    def load_model(self, sess, name=''):
        ...
        return saver.restore(sess, path[:-5])